...
public interface IEvolutionTracker
{
    ...
    /// <param name="profile">The workflow profile containing snapshot configuration.</param>
    /// <returns>The evolution result containing all steps where the component was found.</returns>
    Task<EvolutionResult> GetEvolutionAsync(string componentName, WorkflowProfile profile);
    ...
    Task<TimelineResult> GetTimelineAsync(WorkflowProfile profile);
}